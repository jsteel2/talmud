...
class Tokenizer:
    ...
    def ident_token(self):
        ...
        match ident:
            ...
            case "switch": return Token(T.Switch, ident)
            case "break": return Token(T.Break, ident)
            case "case": return Token(T.Case, ident)
    ...
    def symbol_token(self):
        ...
        match self.char:
            ...
            case "%":
                if not self.advance() and self.char == "!": return ta(T.ModuloU)
                elif self.char == "$": return ta(T.ModuloS)
                else: self.die(f"Unknown symbol {self.char}")
            case "/":
                if not self.advance() and self.char == "!": return ta(T.SlashU)
                elif self.char == "$": return ta(T.SlashS)
                else: self.die(f"Unknown symbol {self.char}")
            case "*": 
                if not self.advance() and self.char == "!": return ta(T.StarU)
                elif self.char == "$": return ta(T.StarS)
                else: return Token(T.Star)
            case ":": return ta(T.Colon)
            case "?": return ta(T.Ternary)
            ...
            case ">":
                if not self.advance() and self.char == ">": return ta(T.BitwiseShiftRight)
                    ...
                    if not self.advance() and self.char == "!": return ta(T.GreaterEqualsU)
                    elif self.char == "$": return ta(T.GreaterEqualsS)
                    else: self.die(f"Unknown symbol {self.char}")
                else:
                    if self.char == "!": return ta(T.GreaterThanU)
                    elif self.char == "$": return ta(T.GreaterThanS)
                    else: self.die(f"Unknown symbol {self.char}")
            case "<":
                if not self.advance() and self.char == "<": return ta(T.BitwiseShiftLeft)
                elif self.char == "=":
                    if not self.advance() and self.char == "!": return ta(T.LessEqualsU)
                    elif self.char == "$": return ta(T.LessEqualsS)
                    else: self.die(f"Unknown symbol {self.char}")
                else:
                    if self.char == "!": return ta(T.LessThanU)
                    elif self.char == "$": return ta(T.LessThanS)
                    else: self.die(f"Unknown symbol {self.char}")
            case "!":
